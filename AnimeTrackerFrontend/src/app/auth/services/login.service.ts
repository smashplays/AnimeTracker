...
export class LoginService {
  ...
  logout(): Observable<Respuesta> {
    ...
    return this.http.get<Respuesta>(this.URL + 'logout', { headers: headers });
  }
  ...
  auth(): Observable<boolean> {
    ...
    return this.http.get<Respuesta>(this.URL + 'me', { headers: headers }).pipe(
      map((res) => {
        if (res.success) {
          return true;
        }
  ...
  register(data: FormGroup): Observable<Respuesta> {
    return this.http.post<Respuesta>(this.URL + 'create', data, {
      headers: this.headers,
    });
  }
}
